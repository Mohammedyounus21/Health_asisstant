    ...
    if condition == "Diabetes":
        ...
        for meal in diabetes_meals:
            ...
            with col1:  # Left side for image and name
                st.image(meal["image"], caption=meal["name"],  use_container_width=True)
...
def create_health_info_card_pdf(name, condition, medications, emergency_contact, specific_instructions):
    ...
    logo_path = "D:\logo.jpg"  # Make sure to have the correct path to the logo file
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        st.image(image, caption='Uploaded Diabetes Risk Factors Report',  use_container_width=True)
...
def validate_diabetes_inputs(inputs):
    ...
    for i, value in enumerate(inputs):
        try:
            ...
            # Specific validation for diabetes-related parameters
            ...
            if i == 1 and float_value > 199:
                return False, ["Glucose level exceeds the maximum limit of 199, that cannot be predictable"]
            if i == 2 and float_value > 122:
                return False, ["Blood Pressure level exceeds the maximum limit of 122, that cannot be predictable"]
            if i == 3 and float_value > 99:
                return False, ["Skin Thickness value exceeds the maximum limit of 99, that cannot be predictable"]
            if i == 4 and float_value > 200:
                return False, ["Insulin level exceeds the maximum limit of 200, meaning the person has Type 2 diabetes & that cannot be predictable"]
            if i == 5 and float_value > 67:
                return False, ["BMI value exceeds the maximum limit of 67, that cannot be predictable"]
            if i == 6 and float_value > 2.42:
                return False, ["Diabetes Pedigree Function value exceeds the maximum limit of 2.42, that cannot be predictable"]
            if i == 7 and float_value > 120:
                return False, ["Age value exceeds the maximum limit of 120, that cannot be predictable"]
...
if selected == 'Diabetes Management':
    ...
    with col2:
        Insulin = st.text_input("Insulin level(microunits/ml)")
    with col3:
    ...
    if st.button('Diabetes Test Result', key='diabetes_button'):
        ...
        # Validate the inputs
        valid, feedback = validate_diabetes_inputs(user_input)
        ...
        if valid:
            ...
            if diab_prediction[0] == 1:
                ...
                *Health Recommendations for Diabetes:*
                ...
                with col5:
                    st.image("https://static.nike.com/a/images/f_auto/dpr_1.0,cs_srgb/w_1824,c_limit/e1e31cdf-f2ab-4f1a-9610-cde7907b054b/the-top-7-resistance-band-chest-exercises-according-to-trainers.jpg", caption="Resistance Band Exercises", use_column_width=True)
                    st.write("Using resistance bands helps build strength and improves insulin sensitivity. It’s also a low-impact option that’s easy on joints.")
                ...
                with col8:
                    st.image("https://everydaypower.com/wp-content/uploads/2016/11/Benefits-of-Jumping-Rope-During-Your-Morning-Routine.jpg", caption="Jump Rope", use_column_width=True)
                    st.write("Great for a quick, high-intensity workout that boosts cardiovascular health and helps in managing weight.")
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        st.image(image, caption='Uploaded Heart Disease Risk Factors Report',  use_container_width=True)

        # Perform analysis on the uploaded report (mock analysis for demonstration)
        analysis_results = "The analysis of the report is based on the uploaded heart disease risk factors."
        worst_thing = "The LDL cholesterol level is significantly elevated, which increases heart disease risk."
        good_thing = "The HDL cholesterol level is within a healthy range."
        suggestion = "Focus on a heart-healthy diet and regular exercise. Consider a consultation with a cardiologist."
...
def validate_heart_disease_inputs(inputs):
    invalid_values = []  # To store invalid input values
    valid = True
    ...
    for i, value in enumerate(inputs):
        try:
            ...
            if i == 2 and float_value not in [0, 1, 2, 3]:
                return False, ["Entered values are invalid, The number cannot exceed more than 3"]
            if i == 3 and (float_value < 90 or float_value > 200):
                return False, ["The trestbps should be between 90 and 200."]
            if i == 4 and (float_value < 125 or float_value > 560):
                return False, ["The chol should be between 125 and 560."]
            if i == 5 and float_value not in [0, 1]:
                return False, ["The Entered values are invalid, 1=true, 0=false"]
            if i == 6 and float_value not in [0, 1, 2]:
                return False, ["The Entered values are invalid, restecg exceed the limit more than 2."]
            if i == 7 and (float_value < 70 or float_value > 200):
                return False, ["Thalach should be between 70 and 120"]
            if i == 8 and float_value not in [0, 1]:
                return False, ["Entered value are invalid, 1=true; 0=false."]
            if i == 9 and (float_value < 0 or float_value > 6.2):
                return False, ["oldpeak should be between 0 and 6.2."]
            if i == 10 and float_value not in [0, 1, 2]:
                return False, ["The Entered values are invalid, restecg exceed the limit more than 2."]
            if i == 11 and float_value not in [0, 1, 2]:
                return False, ["The Entered values are invalid, ca exceed the limit more than 2."]
            if i == 12 and float_value not in [0, 1, 2, 3]:
                return False, ["The Entered values are invalid, thal exceed the limit more than 3."]
...
if selected == 'Heart Disease Management':
    ...
    if st.button('Heart Disease Test Result', key='heart_button'):
        ...
        user_input_hd = np.array(user_input_hd, dtype=float)

        # Validate the inputs
        valid_hd, feedback_hd = validate_heart_disease_inputs(user_input_hd)
        ...
        if valid_hd:
            ...
            if heart_prediction[0] == 1:
                ...
                *Health Recommendations for Heart Disease:*
                ...
                def categorize_heart_risk(value, low_threshold, high_threshold):
                    if value < low_threshold:
                        return 'Low Risk'
                    elif value > high_threshold:
                        return 'High Risk'
                ...
                def visualize_heart_disease_risk(factors, values, low_thresholds, high_thresholds, title="Heart Disease Risk Analysis"):
                    # Categorize each factor based on thresholds
                    categories = [categorize_heart_risk(value, low, high) for value, low, high in zip(values, low_thresholds, high_thresholds)]
                    ...
                    fig = px.bar(
                        ...
                        y=values,
                        color=categories,
                        labels={'x': 'Heart Disease Risk Factors', 'y': 'Values', 'color': 'Risk Category'},
                        title=title,
                        color_discrete_map={
                            'Low Risk': 'green',
                            'Normal': 'blue',
                    ...
                    st.plotly_chart(fig)
                # Defining risk thresholds
                risk_factors = ['Age', 'Sex', 'CP', 'Trestbps', 'Cholestrol', 'FBS', 'RestEcg', 'Thalach', 'Exang','Oldpeak', 'Slope', 'CA', 'Thal']
                categories = [
                    categorize_risk(float(age), 18, 70),
                    categorize_risk(float(sex), 0, 1),
                    categorize_risk(float(cp), 0, 1),
                    categorize_risk(float(trestbps), 90, 130),
                    categorize_risk(float(chol), 125, 275),
                    categorize_risk(float(fbs), 0, 1),
                    categorize_risk(float(restecg), 0, 1),
                    categorize_risk(float(thalach), 70, 155 ),
                    categorize_risk(float(exang), 0, 1),
                    categorize_risk(float(oldpeak), 0, 1.8 ),
                    categorize_risk(float(slope), 0, 1),
                    categorize_risk(float(ca), 0, 1),
                    categorize_risk(float(thal), 0, 2)
                    
                ]
                
                # Visualize with risk categories
                visualize_risk_factors_with_categories(risk_factors, user_input_hd, categories, "Heart Disease Risk Factors")
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        # Display the uploaded image
        st.image(image, caption="Uploaded Parkinson's Disease Risk Factors Report",  use_container_width=True)
...
if selected == "Parkinson's Management":
        ...
        if valid_pd:
            ...
            if parkinsons_prediction[0] == 1:
                ...
                *Health Recommendations for Parkinson's Disease:*
                ...
                with col1:
                    st.image("https://assistinghands.com/34/wp-content/uploads/sites/59/2019/10/Fotolia_78775833_Subscription_Monthly_M.jpg", caption="Big Movements Training (BIG Therapy)",  use_container_width=True)
                ...
                with col2:
                    st.image("https://i.ytimg.com/vi/jWItbongRzQ/maxresdefault.jpg", caption="Marching In Place",  use_container_width=True)
                ...
                with col6:
                    st.image("https://th.bing.com/th/id/OIP.4Bm7Duuhp5BsN95fl6-sfAAAAA?rs=1&pid=ImgDetMain", caption="Cycling",  use_container_width=True)
                    st.write("A great aerobic exercise that improves cardiovascular health without putting too much stress on joints.")
                    
                with col7:
                    st.image("https://th.bing.com/th/id/OIP.Ol1CPHgk2gnjb7iLZM9I4QHaHa?rs=1&pid=ImgDetMain", caption="Step Aerobics",  use_container_width=True)
                    st.write("Utilizing a small step or platform, this workout improves cardiovascular health and leg strength and can be adapted for different fitness levels.")
                
                with col8:
                    st.image("https://www.fisioaviles.com/wp-content/uploads/2019/08/Beneficios-del-pilates.jpg", caption="Pilates",  use_container_width=True)
                    st.write("Strengthens core muscles, improves flexibility, and can be modified to suit different fitness levels, making it ideal for those new to exercise or with limited mobility.")
                
                with col9:
                    st.image("https://www.fitnesseducation.edu.au/wp-content/uploads/2020/10/Walking-stairs-e1587693663738.jpg", caption="Stair Climbing",  use_container_width=True)
                    st.write("Improves leg strength and cardiovascular health; it’s a convenient exercise that can be done anywhere with stairs.")
    ...
    if canvas_result.image_data is not None:
        st.image(canvas_result.image_data, caption="Your handwriting",  use_container_width=True)
...
# Replace "path_to_your_image.jpg" with the actual image file path
    st.image("/Users/younu/Downloads/images.jpg", 
    ...
    if condition == "Diabetes":
        ...
        for idx, exercise in enumerate(diabetes_exercises):
            ...
            with cols[row][col]:  # Select the appropriate cell in the grid
                st.image(exercise["image"], caption=exercise["name"],  use_container_width=True)
